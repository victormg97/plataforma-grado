-- 095: Add color_calendario to profesor object in get_admin_prefetch RPC
-- The horarios_calendario section now returns color_calendario for each profesor
-- so the calendar can display user-chosen colors from the initial prefetch.
...
BEGIN
  ...
  -- horarios_calendario: now includes color_calendario for each profesor
  SELECT COALESCE(jsonb_agg(to_jsonb(s) ORDER BY s.fecha ASC),'[]'::jsonb) INTO v_horarios_cal FROM (
    SELECT h.*,
      ...
      (SELECT jsonb_build_object('id',pr.id,'nombre',pr.nombre,'apellido',pr.apellido,'avatar_url',pr.avatar_url,'color_calendario',pr.color_calendario) FROM profiles pr WHERE pr.id=h.profesor_id) AS profesor,
