  ...
  const dropdown = open && mounted && typeof document !== 'undefined'
    ? createPortal(
        <div
          ref={listRef}
          style={dropdownStyle}
          className="rounded-[var(--radius-md)] border border-[var(--color-border)] bg-[var(--color-bg)] shadow-[var(--shadow-lg)] ring-1 ring-[var(--color-brand-gold)]/20 overflow-hidden"
          role="listbox"
        >
          {options.map((opt) => (
            <button
  ...
  return (
    <>
      <button
        ...
        className={`flex items-center justify-between gap-2 rounded-[var(--radius-md)] border border-[var(--color-border)] bg-[var(--color-bg)] px-3 py-2 text-sm text-left transition-colors hover:border-[var(--color-brand-gold)] focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-brand-gold)] ${open ? 'border-[var(--color-brand-gold)] ring-1 ring-[var(--color-brand-gold)]/30' : ''} ${className}`}
      >
