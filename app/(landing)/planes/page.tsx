...
/**
 * Ruta "/planes". La sección vive dentro de la página principal con scroll
 * continuo, así que esta URL redirige al ancla correspondiente.
 */
export default async function PlanesRoute() {
  if (!tenantConfig.landingPage?.habilitado || !tenantHasLanding()) {
    redirect('/login');
  }

  if (!tenantHasSection('planes')) {
    notFound();
  }

  redirect('/landing#planes');
}
